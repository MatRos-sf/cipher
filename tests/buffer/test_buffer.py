...
class TestBuffer:
    ...
    def test_add_should_not_add_text_when_obj_invalid(self, text):
    ...
    def test_add_list_of_dict_when_arg_is_valid(self, get_dict_of_str):
        ...
        arr_text = [get_dict_of_str for _ in range(5)]
    ...
    @pytest.mark.parametrize("arr", [([1, 2, 3]), (['t', 'e', 's', 't']), ([dict(), dict(), dict()])])
    ...
    def test_print_buffer_should_print_info_when_data_is_empty(self):
        ...
        mock.assert_has_calls(
            [call("Buffer is empty.\n")]
        )

    def test_print_buffer_should_print_all_buffer_when_buffer_is(self, mocker, get_sample_text_obj):
        b = Buffer()
        ...
        mock.assert_has_calls(
            [
                ...
                call(f"{'1':<4}  {shorten(data.text, width=100):<100}  {data.rot_type:<10}  {data.status:<15}"),
                call()
             ]
        )
